#!/usr/bin/env bash
# init-aluno.sh — emite cert Let's Encrypt pro subdomínio do SPA do aluno.
#
# Roda DEPOIS do init-letsencrypt.sh (que já subiu nginx + certbot).
# Adiciona o cert do ALUNO_DOMAIN sem mexer no cert do DJANGO_DOMAIN existente.
#
# Idempotente — se já tem cert real, só sai.
#
# Pré-requisitos:
#   - init-letsencrypt.sh já rodou com sucesso (containers rodando).
#   - DNS do ALUNO_DOMAIN apontando pro IP da VPS (verifique com `dig +short ${ALUNO_DOMAIN}`).
#   - Build do frontend já rsyncado pro volume `aluno_dist` (via deploy-aluno.sh)
#     OU o /srv/aluno pode estar vazio nesse momento (o nginx só serve um 404
...
if [[ -z "$DOMAIN" || -z "$EMAIL" ]]; then
    echo "ERRO: defina ALUNO_DOMAIN e LETSENCRYPT_EMAIL no .env.prod" >&2
    exit 1
fi

# ALUNO_DOMAIN tem que ser FQDN puro (ex.: aluno.foo.com), sem protocolo.
...
if [[ "$DOMAIN" == *://* ]]; then
    echo "ERRO: ALUNO_DOMAIN não pode ter protocolo. Valor atual: '$DOMAIN'" >&2
    echo "      Edite .env.prod removendo 'http://' ou 'https://' do começo." >&2
    echo "      Ex. correto: ALUNO_DOMAIN=aluno.seudominio.com.br" >&2
    exit 1
fi
...
# Cria cert dummy só pra nginx aceitar o server-block 443 do aluno
...
echo "    bash deploy/scripts/deploy-aluno.sh <ssh-alias-da-vps>"
